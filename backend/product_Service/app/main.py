# week09/backend/product_service/app/main.py
...
STOCK_DEDUCTION_TOTAL = Counter(
    ...
    "Total stock deduction attempts",
    ["app_name", "product_id", "status"],
    registry=registry,
)
STOCK_LEVEL_GAUGE = Gauge(
    "product_stock_quantity",
    "Current stock quantity of a product",
    ["app_name", "product_id", "product_name"],
    registry=registry,
)
IMAGE_UPLOAD_TOTAL = Counter(
    "product_image_upload_total",
    "Total product image upload attempts",
    ["app_name", "product_id", "status"],
    registry=registry,
)
LOW_STOCK_ALERTS_TOTAL = Counter(
    "low_stock_alerts_total",
    "Total alerts triggered for low stock",
    ["app_name", "product_id", "product_name"],
    registry=registry,
)
...
async def add_process_time_header(request: Request, call_next):
    ...
    REQUESTS_IN_PROGRESS.labels(
        app_name=APP_NAME, method=method, endpoint=endpoint
    ).inc()
    ...
    REQUEST_COUNT.labels(
        app_name=APP_NAME, method=method, endpoint=endpoint, status_code=status_code
    ).inc()
...
async def add_product_stock(
    ...
    db: Session = Depends(get_db),  # Reusing StockDeductRequest for quantity
):
    """
